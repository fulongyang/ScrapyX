...
from Scrapy_RedisSpider.items import MaiGooItem, get_trademard_fid_id_fid_name, get_region_name_id,GetMysql
start = '*'*10

# class Maigoospiderv2Spider(scrapy.Spider):
class Maigoospiderv2Spider(RedisSpider):
    name            = 'MaiGooSpiderV2'
    allowed_domains = ['maigoo.com']
    
    
    # start_urls      = ['https://www.maigoo.com/brand/search/?catid=7']
    redis_key       = 'MaiGooSpiderV2:RequestQueueV2'
    ...
    set_url         = set()
    classify_dict   = {'2729': '大品牌', '4636': '著名商标', '4635': '省市名牌', '2730': '驰名保护', '2731': '中华老字号', '4630': '高新技术企业', '2732': '上市公司', '4813': '500强企业', '2733': '中小企业'}
    ...
    def parse(self, response):
        ...
        for four_num in four_classify_num:
            ...
            for first_num in first_classify_num:
                ...
                #'横向  切换下一页'
                first_next_link = self.next_linke.format(catid=first_num, classify=classify, page=1)
                if first_next_link not in self.set_url:
                    ...
                    yield scrapy.Request(url=first_next_link, callback=self.next_page, dont_filter=True,
                                         meta={'classify_name': '', 'catid': first_num, 'classify_id':classify, })
            ...
            for two_num in two_classify_num[1:]:
                ...
                #'纵向 切换下一2分类， 回调给下一页,'
                tow_next_link = self.next_linke.format(catid=two_num, classify=classify, page=1)
                if tow_next_link not in self.set_url:
                    ...
                    yield scrapy.Request(url=tow_next_link, callback=self.next_page, dont_filter=False,
                                     meta={ 'catid': two_num,'classify_name': '','classify_id':classify,})
            ...
            if three_classify:
                for three_num in three_classify[1:]:
                    ...
                    three_next_link = self.next_linke.format(catid=three_num, classify=classify, page=1)
                    if three_next_link not in self.set_url:
                        ...
                        yield scrapy.Request(url=three_next_link, callback=self.next_page, dont_filter=False,
                                         meta={'classify':'', 'catid':three_num,'classify_id':classify,})
    ...
    def next_page(self, response):
        ...
        index = response.xpath('//div[@class="num"]/text()').getall()
        for item_index,more_next_pp_link,title in zip(index,more_next_pp_links,titles):
            ...
            if more_next_pp_link not in self.set_url:
                ...
                #------------------mysql 去重
                get_mysql = GetMysql()
                if get_mysql.get_one('title',title):
                    get_mysql.update_id(item_index,title)
                    print(start,title,'is in mysql',start)
                    continue
                get_mysql.close_mysql()
                print(start, title, 'not in  mysql', start)
                #------------------mysql 去重 end
                
                yield scrapy.Request(url=more_next_pp_link, callback=self.parse_response_item, dont_filter=False,
                                 meta={'index_item':item_index,'classify_id':classify_id,})
    ...
    def parse_item_comment(self,response):
        ...
        user_comment = list(map(str.strip,response.xpath('//div[contains(@class,"usersay")]').xpath('string(.)').extract()))
